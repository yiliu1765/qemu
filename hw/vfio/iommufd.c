                                        ...
                                        Error **errp)
{
    ...
    ret = iommufd_backend_alloc_hwpt(bind.iommufd, bind.out_devid,
                                     container->ioas_id, 0,
    ...
    if (ret) {
        ...
        error_setg_errno(errp, errno,
                         "[iommufd=%d] error alloc hwpt linked to ioasid=%d",
                         container->be->fd, attach_data.pt_id);
        return ret;

    }

    attach_data.pt_id = hwptid;
