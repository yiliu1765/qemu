...
static bool vtd_check_sva_info(IntelIOMMUState *s,
...
static bool vtd_sync_sva_info(IntelIOMMUState *s,
                                  struct iommu_sva_info *info)
{
    struct iommu_sva_info_vtd *vtd;
    uint64_t cap, ecap;

    vtd =  (struct iommu_sva_info_vtd *) &info->vendor.vtd;

    if (s->cap_finalized) {
        return vtd_check_sva_info(s, info, vtd);
    }
...
{
    struct iommu_sva_info *info = iommu_ctx->info;
    uint32_t minsz, size;
    ...
    size = sizeof(struct iommu_sva_info_vtd);
    minsz = endof(struct iommu_sva_info, flags);
    ...
    return vtd_sync_sva_info(s, info);
}
