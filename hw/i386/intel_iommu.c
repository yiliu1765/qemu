...
out:
    ...
    entry->translated_addr = vtd_get_slpte_addr(pte, S_AW_BITS) & page_mask;
...
static int vtd_device_attach_hwpt(VTDHostIOMMUDevice *vtd_hdev,
                                  ...
                                  Error **errp)
{
    IOMMUFDDevice *idev = container_of(vtd_hdev->dev, IOMMUFDDevice, base);
    int ret;
    ...
    if (ret) {
        ...
        hwpt->hwpt_id = 0;
        error_setg(errp, "devid %d pasid %d failed to attach hwpt %d",
                                       ...
                                       Error **errp)
{
    ...
    ret = vtd_device_attach_hwpt(vtd_hdev, rid_pasid, pe, s2_hwpt, hwpt, &err);
    if (ret) {
                                  ...
                                  struct iommu_hwpt_vtd_s1_invalidate *cache)
{
    ...
    idev = container_of(vtd_hdev->dev, IOMMUFDDevice, base);
                                 ...
                                 VTDPASIDCacheInfo *pc_info)
{
    ...
    /*
     * Step 2: loop all the exisitng vtd_hdev instances.
